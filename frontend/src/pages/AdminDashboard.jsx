...
const ProjectsTab = () => {
  ...
  return (
    <div className="admin-tab">
      <div className="admin-tab_header">
        <h2 className="admin-tab_title">Projects <span className="admin-count">{projects.length}</span></h2>
        <button className="btn btn-primary" onClick={openCreate}>+ Add Project</button>
      </div>

      {showForm && (
        <form className="admin-form" onSubmit={handleSubmit}>
          <h3 className="admin-form_title">{editing ? 'Edit Project' : 'New Project'}</h3>
        ...
        <div className="admin-list">
          ...
          {projects.map(p => (
            <div key={p._id} className="admin-item">
              ...
              <div className="admin-item_info">
                <span className="admin-item_badge">{p.category}</span>
                {p.featured && <span className="admin-item_badge admin-item_badge--accent">Featured</span>}
                <span className="admin-item_title">{p.title}</span>
                <span className="admin-item_sub">{(p.techStack || []).slice(0, 4).join(' · ')}</span>
              </div>
              <div className="admin-item_actions">
...
const SkillsTab = () => {
  ...
  const openCreate = () => { setForm(blank); setEditing(null); setShowForm(true); };
  const openEdit = (s) => { setForm({ name: s.name, category: s.category, level: s.level }); setEditing(s._id); setShowForm(true); };
  ...
  return (
    <div className="admin-tab">
      <div className="admin-tab_header">
        <h2 className="admin-tab_title">Skills <span className="admin-count">{skills.length}</span></h2>
        <button className="btn btn-primary" onClick={openCreate}>+ Add Skill</button>
      </div>

      {showForm && (
        <form className="admin-form" onSubmit={handleSubmit}>
          <h3 className="admin-form_title">{editing ? 'Edit Skill' : 'New Skill'}</h3>
          <div className="admin-form_grid">
            <div className="admin-field">
        ...
        <div className="admin-list">
          ...
          {skills.map(s => (
            <div key={s._id} className="admin-item">
              ...
              <div className="admin-item_info">
                <span className="admin-item_badge">{s.category}</span>
                <span className="admin-item_title">{s.name}</span>
                <span className="admin-item_sub">{s.level}%</span>
              </div>
              <div className="admin-item_actions">
                <button className="admin-btn-icon" onClick={() => openEdit(s)} title="Edit">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button className="admin-btn-icon admin-btn-icon--danger" onClick={() => setConfirmId(s._id)} title="Delete">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
                </button>
              </div>
            </div>
...
const ExperienceTab = () => {
  ...
  return (
    <div className="admin-tab">
      <div className="admin-tab_header">
        <h2 className="admin-tab_title">Experience <span className="admin-count">{experiences.length}</span></h2>
        <button className="btn btn-primary" onClick={openCreate}>+ Add Experience</button>
      </div>

      {showForm && (
        <form className="admin-form" onSubmit={handleSubmit}>
          <h3 className="admin-form_title">{editing ? 'Edit Experience' : 'New Experience'}</h3>
          <div className="admin-form_grid">
            <div className="admin-field">
        ...
        <div className="admin-list">
          ...
          {experiences.map(exp => (
            <div key={exp._id} className="admin-item">
              ...
              <div className="admin-item_info">
                {exp.current && <span className="admin-item_badge admin-item_badge--accent">Current</span>}
                <span className="admin-item_title">{exp.role} @ {exp.company}</span>
                <span className="admin-item_sub">{fmt(exp.startDate)} — {exp.current ? 'Present' : fmt(exp.endDate)}</span>
              </div>
              <div className="admin-item_actions">
                <button className="admin-btn-icon" onClick={() => openEdit(exp)} title="Edit">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button className="admin-btn-icon admin-btn-icon--danger" onClick={() => setConfirmId(exp._id)} title="Delete">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
                </button>
              </div>
            </div>
  ...
  return (
    <div className="admin-layout">
      <aside className="admin-sidebar">
        <div className="admin-sidebar_brand">
          <span className="admin-sidebar_logo">&lt;Admin /&gt;</span>
        </div>
        <nav className="admin-sidebar_nav">
          {TABS.map(tab => (
            <button
              key={tab}
              className={`admin-sidebar_link ${activeTab === tab ? 'active' : ''}`}
        ...
        <div className="admin-sidebar_footer">
          <p className="admin-sidebar_user">{admin?.email}</p>
          <button className="admin-sidebar_link admin-sidebar_link--logout" onClick={handleLogout}>Logout</button>
          <a href="/" className="admin-sidebar_link admin-sidebar_link--back">← View Portfolio</a>
        </div>
      </aside>
